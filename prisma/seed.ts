...
function getDateTimeFields(modelAccessorName: string): Set<string> {
  ...
  const modelName = modelAccessorName.charAt(0).toUpperCase() + modelAccessorName.slice(1);
  const model = Prisma.dmmf.datamodel.models.find((m) => m.name === modelName);

  if (!model) return new Set();

  return new Set(model.fields.filter((field) => field.type === "DateTime").map((field) => field.name));
}
...
function normalizeRow(row: Record<string, unknown>, dateTimeFields: Set<string>): Record<string, unknown> {
...
async function seedTable(table: TableFixture) {
  ...
  if (!model) {
    console.warn(`⚠️  Não existe um model do Prisma chamado "${table.name}" — pulando.`);
    return;
  }
  ...
  console.log(`✅ ${table.name}: ${table.rows.length} registro(s) inserido(s).`);
}
